...
namespace Rogue_Game.Characters
{
    class UndeadWarrior:Character
    {
        ...
        public UndeadWarrior()
            : base()
        {
            float dmgMin = randed.Instance.rand.Next(415, 460);
            float dmgMax = randed.Instance.rand.Next(530, 578);
            float dodge = randed.Instance.rand.Next(0,30);
            int armor = randed.Instance.rand.Next(65, 130);
            float health = randed.Instance.rand.Next(690, 780);
            float mana = 0;
            magic = false;
            melee = true;
            range = false;
        ...
        override public void draw(Graphics g,Point p)
        {
            ...
            test = Image.FromFile(@"Resources\UndeadWarrior.gif");
        ...
        {
            return randed.Instance.rand.Next(2500, 3000);
